...
slider.oninput = function () { //监听滑动条
    player.value = slider.value;
}
document.addEventListener("input", function () { //监听Input内的数值变化
    destribute();
...
function shuffle(arr) { //新赋值的参数arr，在监听内与playerArr交接
    ...
    for (i = arr.length; i > 0; i--) {
        ...
        // swap(arr, i - 1, randomIndex); //下面封装好的交换函数，进行随机对调
    }
